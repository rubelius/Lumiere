...
Maximize, Volume2, SkipBack, SkipForward, Pause, Music, AlertCircle } from 
"lucide-react"; 
import Link from "next/link"; 
import { useRouter, useParams } from "next/navigation"; 
import { MovieCard } from "@/components/ui/movie-card"; 

// Hook de busca do banco de dados
...
// COMPONENTE DE AVISO DE DADOS FALTANTES
...
export default function MovieDetail() { 
  ...
  // Busca os dados do banco de dados usando o ID da URL
  const { data: movie, isLoading, error } = useMovie(movieId);

  // ── TRATAMENTO DE ESTADOS CINEMATOGRÁFICOS ──
  if (isLoading) {
    return (
  ...
  // ── MAPEAMENTO DINÂMICO DOS DADOS E FALLBACKS ──
  ...
  const posterUrl = movie.poster_url || "/images/poster-1.png"; // Mantém o placeholder visual se o back não mandar imagem
  const backdropUrl = movie.backdrop_url || "/images/hero-backdrop.png";

  // Arrays mapeados direto do backend (se for null/undefined, vira array vazio para triggar o MissingData)
  const cast = movie.cast || [];
  const festivals = movie.festivals || [];
  const reviews = movie.reviews || [];
  const technicalSpecs = movie.technical_specs || [];
          ...
          > 
            ...
            > 
              <div style={{ position: 'absolute', inset: 0, zIndex: 0 }}> 
                <img src={backdropUrl} style={{ width: '100%', height: '100%', objectFit: 'cover', filter: 'grayscale(100%) contrast(125%)', opacity: 0.6 }} alt="Video Poster" /> 
              </div> 
              ...
              <div style={{ position: 'absolute', top: 0, left: 0, right: 0, padding: '24px', display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', zIndex: 10, background: 'linear-gradient(to bottom, rgba(4,4,2,0.9), transparent)', opacity: 0, transition: 'opacity 0.3s' }} className="group-hover/player:opacity-100"> 
                <motion.div initial={{ y: -10, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ delay: 0.2 }}> 
                  ...
                  <h3 style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: '2rem', color: '#EDE8DC', margin: 0 }}>{movie.title} - Trailer Oficial</h3> 
                </motion.div> 
                <motion.button  
                  onClick={() => setIsTrailerOpen(false)}  
              ...
              <div style={{ position: 'absolute', bottom: 0, left: 0, right: 0, padding: '32px', zIndex: 10, background: 'linear-gradient(to top, rgba(4,4,2,0.95), transparent)', opacity: 0, transition: 'opacity 0.3s', display: 'flex', flexDirection: 'column', gap: '24px' }} className="group-hover/player:opacity-100"> 
                ...
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}> 
                  <div style={{ display: 'flex', alignItems: 'center', gap: '32px' }}> 
                    ...
                    <span style={{ fontFamily: "'DM Mono', monospace", fontSize: '9px', color: '#565450', letterSpacing: '0.1em', marginLeft: '16px' }}>00:00 / {runtime}</span> 
                  </div> 
                   
                  <div style={{ display: 'flex', alignItems: 'center', gap: '24px' }}> 
                    <motion.button whileHover={{ color: '#EDE8DC' }} whileTap={{ scale: 0.95 }} style={{ background: 'transparent', border: 'none', color: '#8C8880', fontFamily: "'DM Mono', monospace", fontSize: '9px', letterSpacing: '0.15em', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '8px' }}><Settings2 style={{ width: 16, height: 16 }} /> [ AUDIO ]</motion.button> 
                    <motion.button whileHover={{ color: '#EDE8DC' }} whileTap={{ scale: 0.95 }} style={{ background: 'transparent', border: 'none', color: '#8C8880', fontFamily: "'DM Mono', monospace", fontSize: '9px', letterSpacing: '0.15em', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '8px' }}><Subtitles style={{ width: 16, height: 16 }} /> [ LEG ]</motion.button> 
                    <div style={{ height: '16px', width: '1px', backgroundColor: 'rgba(86,84,80,0.3)' }} /> 
                    <span style={{ fontFamily: "'DM Mono', monospace", fontSize: '9px', color: '#BF8F3C', letterSpacing: '0.15em', border: '1px solid #BF8F3C', padding: '4px 8px', backgroundColor: 'rgba(191,143,60,0.1)' }}>NATIVE</span> 
            ...
            <motion.div initial={{ opacity: 0, y: 40 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, delay: 0.1, ease: FINE_ART_EASE }} style={{ width: '320px', flexShrink: 0 }}> 
              ...
              <motion.div whileHover={{ y: -5, boxShadow: '0 20px 60px rgba(0,0,0,1)' }} transition={{ duration: 0.4, ease: FINE_ART_EASE }} style={{ position: 'relative', aspectRatio: '2/3', backgroundColor: '#040402', border: '1px solid rgba(191,143,60,0.3)', padding: '8px', marginBottom: '32px', overflow: 'hidden', boxShadow: '0 0 50px rgba(0,0,0,0.8)' }} className="group"> 
                <div style={{ position: 'relative', width: '100%', height: '100%', border: '1px solid rgba(86,84,80,0.3)', overflow: 'hidden' }}> 
                  <img src={posterUrl} alt="Poster" style={{ width: '100%', height: '100%', objectFit: 'cover', filter: 'grayscale(100%) contrast(125%)', transition: 'all 0.7s' }} className="group-hover:grayscale-0 group-hover:scale-105" /> 
            ...
            <div style={{ flex: 1, minWidth: '300px', maxWidth: '1000px', paddingTop: '16px' }}> 
              ...
              <motion.div variants={staggerContainer} initial="hidden" animate="visible"> 
                <motion.div variants={fadeUpItem} style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'center', gap: '16px', marginBottom: '24px' }}> 
                  ...
                  {movie.current_ranking && (
                    <motion.span whileHover={{ scale: 1.05, borderColor: '#EDE8DC', color: '#EDE8DC' }} whileTap={{ scale: 0.95 }} style={{ cursor: 'crosshair', transition: 'all 0.3s', fontFamily: "'DM Mono', monospace", fontSize: '9px', letterSpacing: '0.2em', padding: '6px 12px', border: '1px solid #565450', color: '#8C8880', display: 'flex', alignItems: 'center', gap: '8px', textTransform: 'uppercase' }}> 
                      <Award style={{ width: 12, height: 12 }} /> RANKING #{movie.current_ranking} 
                    </motion.span> 
                  )}
                </motion.div> 
                ...
                <motion.div variants={fadeUpItem} style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'center', gap: '24px', color: '#8C8880', marginBottom: '48px', fontFamily: "'DM Mono', monospace", fontSize: '10px', letterSpacing: '0.2em' }}> 
                  ...
                  <span style={{ display: 'flex', alignItems: 'center', gap: '8px' }}><Star style={{ width: 12, height: 12, color: '#BF8F3C' }} /> {movie.director?.toUpperCase() || 'DIRETOR NÃO REGISTRADO'}</span> 
                ...
                <motion.div variants={fadeUpItem} style={{ marginBottom: '80px', maxWidth: '800px' }}> 
                  ...
                  <p style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: '2rem', lineHeight: 1.6, color: '#8C8880', margin: 0, display: '-webkit-box', WebkitLineClamp: showFullSynopsis ? 'none' : 3, WebkitBoxOrient: 'vertical', overflow: 'hidden' }}> 
                    {movie.synopsis || "O registro descritivo desta obra encontra-se indisponível no banco de dados central no momento."} 
                  </p> 
                  {movie.synopsis && (
              ...
              <AnimatePresence mode="wait"> 
                {activeTab === "overview" && ( 
                  <motion.div key="overview" variants={staggerContainer} initial="hidden" animate="visible" exit={{ opacity: 0, y: -10 }} transition={{ duration: 0.3 }} style={{ display: 'flex', flexDirection: 'column', gap: '96px' }}> 
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '48px' }}> 
                      <motion.div variants={fadeUpItem} whileHover={{ y: -4, backgroundColor: 'rgba(237,232,220,0.02)' }} style={{ display: 'flex', alignItems: 'flex-start', gap: '24px', padding: '16px', borderRadius: '4px', transition: 'background-color 0.3s' }} className="group cursor-crosshair"> 
                        <div style={{ width: '80px', aspectRatio: '3/4', backgroundColor: '#040402', border: '1px solid rgba(86,84,80,0.3)', padding: '4px', flexShrink: 0 }}> 
                          <img src={movie.director_img || "https://i.pravatar.cc/150?u=placeholder"} alt="Director" style={{ width: '100%', height: '100%', objectFit: 'cover', filter: 'grayscale(100%) contrast(125%)', transition: 'all 0.5s' }} className="group-hover:grayscale-0 group-hover:scale-105" /> 
                        </div> 
                        <div> 
                          ...
                          <p style={{ fontFamily: "'DM Mono', monospace", fontSize: '9px', color: '#8C8880', lineHeight: 1.6, margin: 0, letterSpacing: '0.05em', textTransform: 'uppercase' }}>{movie.director_description || 'Sem descrição biográfica arquivada.'}</p> 
                        </div> 
                      </motion.div> 
                      ...
                      <motion.div variants={fadeUpItem} whileHover={{ y: -4, backgroundColor: 'rgba(237,232,220,0.02)' }} style={{ display: 'flex', alignItems: 'flex-start', gap: '24px', padding: '16px', borderRadius: '4px', transition: 'background-color 0.3s' }} className="group cursor-crosshair"> 
                        ...
                        <div> 
                          ...
                          <h3 style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: '2.5rem', color: '#EDE8DC', margin: '0 0 12px 0' }}>{movie.dp_name || 'NÃO REGISTRADO'}</h3> 
                          <p style={{ fontFamily: "'DM Mono', monospace", fontSize: '9px', color: '#8C8880', lineHeight: 1.6, margin: 0, letterSpacing: '0.05em', textTransform: 'uppercase' }}>{movie.dp_description || 'Sem registro da direção de fotografia.'}</p> 
                        </div> 
                      </motion.div> 
                    </div> 
                          ...
                          {cast.map((actor: any, i: number) => ( 
                            <motion.div key={i} whileHover={{ y: -4, scale: 1.02 }} whileTap={{ scale: 0.95 }} transition={{ type: 'spring', stiffness: 300 }} style={{ display: 'flex', flexDirection: 'column', gap: '16px' }} className="group cursor-crosshair"> 
                              <div style={{ width: '100%', aspectRatio: '3/4', backgroundColor: '#040402', border: '1px solid rgba(86,84,80,0.3)', padding: '4px', overflow: 'hidden' }}> 
                                <img src={actor.img || "https://i.pravatar.cc/150?u=placeholder"} style={{ width: '100%', height: '100%', objectFit: 'cover', filter: 'grayscale(100%) contrast(125%)', transition: 'all 0.5s' }} className="group-hover:grayscale-0 group-hover:scale-105" alt={actor.name} /> 
                              </div> 
                              <div> 
                    ...
                    {/* Sincronização Sugerida - Só aparece se houver recomendação ativa no DB */}
                      ...
                      {reviews.length > 0 ? (
                        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(400px, 1fr))', gap: '32px' }}> 
                          {reviews.map((review: any, i: number) => ( 
                            <motion.div key={i} whileHover={{ y: -4, borderColor: 'rgba(191,143,60,0.3)', boxShadow: '0 10px 30px rgba(0,0,0,0.5)' }} whileTap={{ scale: 0.98 }} style={{ padding: '32px', border: '1px solid rgba(86,84,80,0.3)', background: 'linear-gradient(to bottom, #040402, #080806)', display: 'flex', flexDirection: 'column', cursor: 'crosshair', transition: 'all 0.3s' }}> 
                        ...
                        <MissingData label="REVIEWS / CRÍTICAS" />
                      )}
                    </motion.div> 
                  ...
                  <motion.div key="media" variants={staggerContainer} initial="hidden" animate="visible" exit={{ opacity: 0, y: -10 }} transition={{ duration: 0.3 }} style={{ display: 'flex', flexDirection: 'column', gap: '96px' }}> 
                    ...
                    <motion.div variants={fadeUpItem}> 
                      ...
                      {mediaCaptures.length > 0 ? (
                        ...
                        <MissingData label="GALERIA DE IMAGENS (BACKDROPS)" />
                      )}
                    </motion.div> 
