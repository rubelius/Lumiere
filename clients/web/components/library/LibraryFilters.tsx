...
export function LibraryFilters({ filters, setFilters, isFilterOpen, setIsFilterOpen }: LibraryFiltersProps) {
  ...
  const FilterGroup = ({ title, options, filterKey }: { title: string, options: string[], filterKey: keyof FilterState }) => (
    <div>
      <div style={{ fontFamily: "'DM Mono', monospace", fontSize: '9px', color: 'var(--gold)', marginBottom: 24, letterSpacing: '0.2em' }}>
        [ {title} ]
      </div>
        ...
        {options.map((opt) => {
          ...
          return (
            <label key={opt} onClick={() => toggleArrayFilter(filterKey, opt)} style={{ display: 'flex', alignItems: 'center', gap: 12, cursor: 'pointer' }}>
              <div style={{ width: 14, height: 14, border: isSelected ? '1px solid var(--gold)' : '1px solid rgba(237,232,220,0.15)', background: isSelected ? 'rgba(191,143,60,0.1)' : 'transparent', display: 'flex', alignItems: 'center', justifyContent: 'center', transition: 'all 0.2s' }}>
                {isSelected && <Check style={{ width: 10, height: 10, color: 'var(--gold)' }} />}
              </div>
              <span style={{ fontFamily: "'DM Mono', monospace", fontSize: '10px', color: isSelected ? 'var(--film)' : 'var(--m3)', letterSpacing: '0.15em', transition: 'color 0.2s' }}>{opt}</span>
            </label>
          )
        })}
      </div>
            ...
            return (
              <button 
                ...
                style={{ 
                  ...
                  color: isActive ? 'var(--gold)' : 'var(--m3)',
                ...
                onMouseLeave={(e) => { e.currentTarget.style.color = isActive ? 'var(--gold)' : 'var(--m3)' }}
              >
                {cat}
              </button>
            )
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 32 }}>
          <div style={{ display: 'flex', gap: 16, fontFamily: "'DM Mono', monospace", fontSize: '9px', letterSpacing: '0.2em', textTransform: 'uppercase' }}>
            <button 
              ...
              style={{ background: 'transparent', border: 'none', cursor: 'pointer', color: filters.viewMode === 'grid' ? 'var(--film)' : 'var(--m3)', transition: 'color 0.3s' }}
            >
              [ GRADE ]
            </button>
            <button 
              onClick={() => setFilters(prev => ({ ...prev, viewMode: "list" }))}
              style={{ background: 'transparent', border: 'none', cursor: 'pointer', color: filters.viewMode === 'list' ? 'var(--film)' : 'var(--m3)', transition: 'color 0.3s' }}
            >
              [ LISTA ]
            </button>
          </div>
          ...
          <div style={{ position: 'relative', width: 280 }}>
            <Search style={{ position: 'absolute', left: 0, top: '50%', transform: 'translateY(-50%)', width: 14, height: 14, color: 'var(--m3)' }} />
            <input 
              type="text" 
              ...
              style={{ 
                ...
                padding: '8px 8px 8px 24px', color: 'var(--film)', fontFamily: "'DM Mono', monospace", fontSize: '10px', 
          ...
          <button 
            ...
            style={{ 
              ...
              color: isFilterOpen ? 'var(--gold)' : 'var(--m2)', 
      ...
      <AnimatePresence>
        {isFilterOpen && (
          <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} transition={{ duration: 0.6, ease: FINE_ART_EASE }} style={{ overflow: 'hidden' }}>
            <div style={{ border: '1px solid rgba(237,232,220,0.05)', background: 'var(--void)', padding: '48px 64px', display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 40 }}>
            ...
            <div style={{ background: 'var(--void)', padding: '0 64px 32px', display: 'flex', justifyContent: 'flex-end', borderLeft: '1px solid rgba(237,232,220,0.05)', borderRight: '1px solid rgba(237,232,220,0.05)', borderBottom: '1px solid rgba(237,232,220,0.05)' }}>
               <button 
                  onClick={() => setFilters(prev => ({ ...prev, qualities: [], genres: [], decades: [], curations: [] }))}
                  style={{ background: 'transparent', border: 'none', color: 'var(--m3)', fontFamily: "'DM Mono', monospace", fontSize: '9px', letterSpacing: '0.15em', cursor: 'pointer', textTransform: 'uppercase' }}
                >
