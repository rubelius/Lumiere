...
export default function MovieClient() { 
  ...
  if (error || !movie) {
    return (
      <div style={{ background: 'var(--bg)', minHeight: '100dvh', display: 'flex', alignItems: 'center', justifyContent: 'center', flexDirection: 'column', gap: '24px' }}>
        <AlertCircle style={{ width: 48, height: 48, color: 'var(--danger)' }} />
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} style={{ fontFamily: "'DM Mono', monospace", fontSize: '10px', color: 'var(--danger)', letterSpacing: '0.2em' }}>
          FALHA NA RECUPERAÇÃO DOS DADOS. DIRETÓRIO CORROMPIDO.
        </motion.div>
        <Link href="/library" style={{ color: 'var(--m2)', fontFamily: "'DM Mono', monospace", fontSize: '9px', letterSpacing: '0.15em', textDecoration: 'none', borderBottom: '1px solid var(--m2)', paddingBottom: '2px' }}>
          [ RETORNAR AO ACERVO ]
        </Link>
      </div>
    );
...
// ── LEITURA BLINDADA DA EQUIPE ──
  ...
  const renderStars = (rating: any) => {
    ...
    const stars = [];
    for(let i=0; i<5; i++) {
      if (i < fullStars) stars.push(<Star key={i} style={{ width: 12, height: 12, color: 'var(--gold)' }} fill="currentColor" />);
      else if (i === fullStars && hasHalf) stars.push(<StarHalf key={i} style={{ width: 12, height: 12, color: 'var(--gold)' }} fill="currentColor" />);
      else stars.push(<Star key={i} style={{ width: 12, height: 12, color: 'var(--m3)' }} />);
    }
    return <div style={{ display: 'flex', gap: '2px' }}>{stars}</div>;
  }

  return ( 
    <div style={{ background: 'var(--bg)', color: 'var(--film)', minHeight: '100dvh', display: 'flex', overflowX: 'hidden', position: 'relative' }}> 
      ...
      <main style={{ flex: 1, minWidth: 0, position: 'relative' }}> 
        ...
        <div style={{ maxWidth: '1400px', margin: '0 auto', padding: '96px 72px 120px', position: 'relative', zIndex: 10 }}> 
          ...
          <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.8, ease: FINE_ART_EASE }} style={{ marginBottom: '64px' }}> 
            <Link href="/library" style={{ display: 'inline-flex', alignItems: 'center', gap: '16px', color: 'var(--m3)', textDecoration: 'none', fontFamily: "'DM Mono', monospace", fontSize: '9px', letterSpacing: '0.2em', textTransform: 'uppercase', transition: 'color 0.3s' }} onMouseEnter={e => e.currentTarget.style.color = 'var(--film)'} onMouseLeave={e => e.currentTarget.style.color = 'var(--m3)'}> 
            ...
            <div style={{ flex: 1, minWidth: '300px', maxWidth: '1000px', paddingTop: '16px' }}> 
              ...
              <motion.div variants={staggerContainer} initial="hidden" animate="visible"> 
                <motion.div variants={fadeUpItem} style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'center', gap: '16px', marginBottom: '32px' }}> 
                  <motion.span whileHover={{ scale: 1.05, borderColor: 'var(--film)', color: 'var(--film)' }} whileTap={{ scale: 0.95 }} style={{ cursor: 'crosshair', transition: 'all 0.3s', fontFamily: "'DM Mono', monospace", fontSize: '9px', letterSpacing: '0.2em', padding: '6px 12px', border: '1px solid var(--gold)', color: 'var(--gold)', display: 'flex', alignItems: 'center', gap: '8px', textTransform: 'uppercase' }}> 
                  ...
                  {movie.current_ranking && (
                    <motion.div className="group/rank" style={{ position: 'relative' }}>
                      <motion.span whileHover={{ scale: 1.05, borderColor: 'var(--film)', color: 'var(--film)', backgroundColor: 'rgba(237,232,220,0.05)' }} whileTap={{ scale: 0.95 }} style={{ cursor: 'crosshair', transition: 'all 0.3s', fontFamily: "'DM Mono', monospace", fontSize: '9px', letterSpacing: '0.2em', padding: '6px 12px', border: '1px solid var(--m3)', color: 'var(--m2)', display: 'flex', alignItems: 'center', gap: '8px', textTransform: 'uppercase' }}> 
                ...
                {movie.original_title && movie.original_title !== movie.title && (
                  <motion.h2 variants={fadeUpItem} style={{ fontFamily: "'DM Mono', monospace", fontSize: '14px', color: 'var(--m3)', letterSpacing: '0.3em', textTransform: 'uppercase', marginBottom: '32px' }}>
                ...
                {movie.streaming_providers && movie.streaming_providers.length > 0 && (
                  <motion.div variants={fadeUpItem} style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '24px' }}>
                    <span style={{ fontFamily: "'DM Mono', monospace", fontSize: '8px', color: 'var(--m2)', letterSpacing: '0.15em', textTransform: 'uppercase' }}>ASSISTIR EM:</span>
                ...
                {movie.tagline && (
                  <motion.h3 variants={fadeUpItem} style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: '2rem', fontStyle: 'italic', fontWeight: 300, color: 'var(--gold)', marginBottom: '16px', margin: '0 0 16px 0' }}> 
                    "{movie.tagline}"
                  </motion.h3>
                )}
                ...
                {movie.alternative_titles && movie.alternative_titles.length > 0 && (
                  <motion.div variants={fadeUpItem} style={{ display: 'flex', gap: '16px', overflowX: 'auto', paddingBottom: '16px', marginBottom: '32px' }} className="scrollbar-none">
                    {movie.alternative_titles.map((alt: any, i: number) => (
                       <span key={i} style={{ flexShrink: 0, fontFamily: "'DM Mono', monospace", fontSize: '8px', color: 'var(--m3)', border: '1px solid rgba(86,84,80,0.3)', padding: '2px 6px', letterSpacing: '0.1em' }}>
                ...
                <motion.div variants={fadeUpItem} style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'center', gap: '24px', color: 'var(--m2)', marginBottom: '48px', fontFamily: "'DM Mono', monospace", fontSize: '10px', letterSpacing: '0.2em' }}> 
                  ...
                  {movie.tmdb_rating && (
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '6px', borderLeft: '1px solid rgba(86,84,80,0.3)', paddingLeft: '24px' }}>
                      <span style={{ fontFamily: "'DM Mono', monospace", fontSize: '8px', color: 'var(--m3)', letterSpacing: '0.1em' }}>[ TMDB AVALIAÇÃO ]</span>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                        {renderStars(movie.tmdb_rating)}
                        <span style={{ color: 'var(--film)' }}>
                          {Number(movie.tmdb_rating).toFixed(1)} <span style={{ color: 'var(--m3)' }}>/ 10</span>
                        </span>
                      </div>
                    </div>
                  )}
                ...
                <motion.div variants={fadeUpItem} style={{ display: 'flex', flexWrap: 'wrap', gap: '16px', marginBottom: '64px' }}> 
                  {movie.genres && movie.genres.length > 0 ? (
                    ...
                    <span style={{ fontFamily: "'DM Mono', monospace", fontSize: '9px', color: 'var(--m3)', letterSpacing: '0.15em' }}>[ GÊNEROS NÃO DEFINIDOS ]</span>
                  )}
                </motion.div> 

                <motion.div variants={fadeUpItem} style={{ marginBottom: '80px', maxWidth: '800px' }}> 
                  <div style={{ fontFamily: "'DM Mono', monospace", fontSize: '9px', color: 'var(--m3)', letterSpacing: '0.2em', marginBottom: '16px' }}>// REGISTRO DE ENREDO</div> 
                  <p style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: '2rem', lineHeight: 1.6, color: 'var(--m2)', margin: 0, display: '-webkit-box', WebkitLineClamp: showFullSynopsis ? 'none' : 3, WebkitBoxOrient: 'vertical', overflow: 'hidden' }}> 
                    {movie.overview || "O registro descritivo desta obra encontra-se indisponível no banco de dados central no momento."} 
                  </p> 
                  {movie.overview && (
                    <motion.button whileHover={{ color: 'var(--film)' }} whileTap={{ scale: 0.95 }} onClick={() => setShowFullSynopsis(!showFullSynopsis)} style={{ marginTop: '24px', backgroundColor: 'transparent', border: 'none', color: 'var(--gold)', fontFamily: "'DM Mono', monospace", fontSize: '9px', letterSpacing: '0.2em', cursor: 'pointer', transition: 'color 0.3s' }}> 
              ...
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: '48px', marginBottom: '64px', borderBottom: '1px solid rgba(86,84,80,0.3)' }}> 
                {[ { id: "overview", label: "01 EQUIPE & ELENCO" }, { id: "details", label: "02 AUDITORIA & METADADOS" }, { id: "media", label: "03 ARQUIVOS DE MÍDIA" } ].map(tab => ( 
                  <button 
                    ...
                    style={{ paddingBottom: '16px', backgroundColor: 'transparent', border: 'none', fontFamily: "'DM Mono', monospace", fontSize: '10px', letterSpacing: '0.2em', cursor: 'pointer', transition: 'color 0.3s', position: 'relative', color: activeTab === tab.id ? 'var(--gold)' : 'var(--m3)' }} 
                    onMouseEnter={e => { if (activeTab !== tab.id) e.currentTarget.style.color = 'var(--m2)' }} 
                    onMouseLeave={e => { if (activeTab !== tab.id) e.currentTarget.style.color = 'var(--m3)' }} 
                  > 
                    {tab.label} 
                    {activeTab === tab.id && <motion.div layoutId="activeTabMovie" transition={{ duration: 0.3 }} style={{ position: 'absolute', bottom: 0, left: 0, right: 0, height: '2px', backgroundColor: 'var(--gold)', boxShadow: '0 0 8px rgba(191,143,60,0.6)' }} />} 
                  </button> 
                ))} 
              </div> 
              ...
              <AnimatePresence mode="wait"> 
                  ...
                  <motion.div key="overview" variants={staggerContainer} initial="hidden" animate="visible" exit={{ opacity: 0, y: -10 }} transition={{ duration: 0.3 }} style={{ display: 'flex', flexDirection: 'column', gap: '96px' }}> 
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '32px' }}> 
                      ...
                      <motion.div variants={fadeUpItem} whileHover={{ y: -4, backgroundColor: 'rgba(237,232,220,0.02)' }} style={{ display: 'flex', alignItems: 'center', gap: '24px', padding: '16px', borderRadius: '4px', transition: 'background-color 0.3s' }} className="group cursor-crosshair"> 
                        ...
                        <div> 
                          <div style={{ fontFamily: "'DM Mono', monospace", fontSize: '9px', color: 'var(--gold)', letterSpacing: '0.2em', marginBottom: '8px' }}>// DIREÇÃO</div> 
                          <h3 style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: '2.2rem', color: 'var(--film)', margin: '0 0 4px 0', lineHeight: 1 }}>{movie.director || 'Desconhecido'}</h3> 
                        </div> 
                      </motion.div> 
                      ...
                      {movie.cinematographer && (
                        <motion.div variants={fadeUpItem} whileHover={{ y: -4, backgroundColor: 'rgba(237,232,220,0.02)' }} style={{ display: 'flex', alignItems: 'center', gap: '24px', padding: '16px', borderRadius: '4px', transition: 'background-color 0.3s' }} className="group cursor-crosshair"> 
                          ...
                          <div> 
                            <div style={{ fontFamily: "'DM Mono', monospace", fontSize: '9px', color: 'var(--gold)', letterSpacing: '0.2em', marginBottom: '8px' }}>// FOTOGRAFIA</div> 
                            <h3 style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: '2.2rem', color: 'var(--film)', margin: '0 0 4px 0', lineHeight: 1 }}>{movie.cinematographer}</h3> 
                          </div> 
                        </motion.div> 
                      )}
                      ...
                      {movie.writer && (
                        <motion.div variants={fadeUpItem} whileHover={{ y: -4, backgroundColor: 'rgba(237,232,220,0.02)' }} style={{ display: 'flex', alignItems: 'center', gap: '24px', padding: '16px', borderRadius: '4px', transition: 'background-color 0.3s' }} className="group cursor-crosshair"> 
                          ...
                          <div> 
                            <div style={{ fontFamily: "'DM Mono', monospace", fontSize: '9px', color: 'var(--gold)', letterSpacing: '0.2em', marginBottom: '8px' }}>// ROTEIRO</div> 
                            <h3 style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: '2.2rem', color: 'var(--film)', margin: '0 0 4px 0', lineHeight: 1 }}>{movie.writer}</h3> 
                          </div> 
                        </motion.div> 
                      )}
                      ...
                      {movie.composer && (
                        <motion.div variants={fadeUpItem} whileHover={{ y: -4, backgroundColor: 'rgba(237,232,220,0.02)' }} style={{ display: 'flex', alignItems: 'center', gap: '24px', padding: '16px', borderRadius: '4px', transition: 'background-color 0.3s' }} className="group cursor-crosshair"> 
                          ...
                          <div> 
                            <div style={{ fontFamily: "'DM Mono', monospace", fontSize: '9px', color: 'var(--gold)', letterSpacing: '0.2em', marginBottom: '8px' }}>// TRILHA SONORA</div> 
                            <h3 style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: '2.2rem', color: 'var(--film)', margin: '0 0 4px 0', lineHeight: 1 }}>{movie.composer}</h3> 
                          </div> 
                        </motion.div> 
                      )}
                    </div>
                    ...
                    <motion.div variants={fadeUpItem}> 
                      <div style={{ fontFamily: "'DM Mono', monospace", fontSize: '9px', color: 'var(--m3)', letterSpacing: '0.2em', marginBottom: '32px' }}>// IDENTIFICAÇÃO DE ELENCO</div> 
                      {cast.length > 0 ? (
                        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(120px, 1fr))', gap: '32px 24px' }}> 
                          {cast.map((actor: any, i: number) => ( 
                            <motion.div key={i} whileHover={{ y: -4, scale: 1.02 }} whileTap={{ scale: 0.95 }} transition={{ type: 'spring', stiffness: 300 }} style={{ display: 'flex', flexDirection: 'column', gap: '16px', alignItems: 'center', textAlign: 'center' }} className="group cursor-crosshair"> 
                              ...
                              <div> 
                                <div style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: '1.6rem', color: 'var(--film)', lineHeight: 1, marginBottom: '6px' }} className="group-hover:text-[var(--gold)] transition-colors">{actor.name}</div> 
                                <div style={{ fontFamily: "'DM Mono', monospace", fontSize: '8px', color: 'var(--m2)', letterSpacing: '0.1em', textTransform: 'uppercase' }}>{actor.role}</div> 
                              </div> 
                            </motion.div> 
                          ))} 
                        </div> 
                ...
                {activeTab === "details" && ( 
                  <motion.div key="details" variants={staggerContainer} initial="hidden" animate="visible" exit={{ opacity: 0, y: -10 }} transition={{ duration: 0.3 }} style={{ display: 'flex', flexDirection: 'column', gap: '96px' }}> 
                    ...
                    <motion.div variants={fadeUpItem}> 
                      <div style={{ fontFamily: "'DM Mono', monospace", fontSize: '9px', color: 'var(--m3)', letterSpacing: '0.2em', marginBottom: '32px' }}>// ESPECIFICAÇÕES TÉCNICAS E FINANCEIRAS</div> 
                        ...
                        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(250px, 1fr))', rowGap: '48px', columnGap: '64px', borderTop: '1px solid rgba(86,84,80,0.3)', borderBottom: '1px solid rgba(86,84,80,0.3)', padding: '48px 0' }}> 
                          {technicalSpecs.map((spec: any, i: number) => ( 
                            <motion.div key={i} whileHover={{ x: 4 }} className="group cursor-crosshair"> 
                              <p style={{ fontFamily: "'DM Mono', monospace", fontSize: '8px', color: 'var(--gold)', letterSpacing: '0.2em', marginBottom: '8px', textTransform: 'uppercase', transition: 'color 0.3s' }} className="group-hover:text-[var(--film)]">{spec.label}</p> 
                              <p style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: '2.5rem', color: 'var(--film)', margin: 0 }}>{spec.value}</p> 
                            </motion.div> 
                          ))} 
                        </div> 
                      ) : (
                ...
                {activeTab === "media" && ( 
                  <motion.div key="media" variants={staggerContainer} initial="hidden" animate="visible" exit={{ opacity: 0, y: -10 }} transition={{ duration: 0.3 }} style={{ display: 'flex', flexDirection: 'column', gap: '96px' }}> 
                    ...
                    <motion.div variants={fadeUpItem}> 
                      <div style={{ fontFamily: "'DM Mono', monospace", fontSize: '9px', color: 'var(--m3)', letterSpacing: '0.2em', marginBottom: '32px' }}>// CAPTURAS ÓPTICAS</div> 
          ...
          <motion.section initial={{ opacity: 0, y: 40 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-10%" }} transition={{ duration: 0.8, ease: FINE_ART_EASE }} style={{ paddingTop: '96px', borderTop: '1px solid rgba(86,84,80,0.3)' }}> 
            <div style={{ fontFamily: "'DM Mono', monospace", fontSize: '9px', color: 'var(--m3)', letterSpacing: '0.2em', marginBottom: '48px' }}>// OBRAS CORRELATAS IDENTIFICADAS</div> 
